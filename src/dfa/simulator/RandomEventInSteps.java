...
public class RandomEventInSteps implements SimulationMode
{
    public Statechart statechart;

    public RandomEventInSteps(Statechart s)
    {
        this.statechart = s;
    }
    public void simulate(ExecutionState eState)
    {
        try
        {
            ...
            State curr = ExhaustiveSimulator.get_atomic_state((State)statechart, 1, eState);                // gets to the atomic-state (i.e., this is like the start state for the statechart)
            ...
            while(true)
            {
              ...
              Transition t = ExhaustiveSimulator.get_valid_transition(curr, curr, 0, null, event, this.statechart, eState);
              System.out.println("Performing transition: " + t.name);
              ExhaustiveSimulator.performTransition(t, curr, this.statechart, eState);
              System.out.println(eState.generate_summary());
              curr = t.getDestination();
              System.out.println("Reached State: " + curr.getFullName());
              System.out.println("+--------------------------------------------------+");
              curr = ExhaustiveSimulator.get_atomic_state(curr, 0, eState);                                 // gets to the state where from where the execution begins
              counter ++;
            }
        }
        catch(Exception e)
