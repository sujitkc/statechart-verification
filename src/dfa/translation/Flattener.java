...
class Globaliser extends Translator {
  ...
  private void makeGlobalDeclarations(State state) {
    for(Declaration dec : state.declarations) {
      ...
      Declaration newdec = new Declaration(
        ...
        dec.typeName,
        dec.input);
		newdec.setType(dec.getType().copy());
        
		System.out.println("Creating new declaration: "+dec.getFullVName()+" : "+underscoredName+" : "+dec.typeName);
  ...
  private Statechart globaliseStatechart(Statechart statechart) throws Exception {
    ...
    for(Declaration declaration : this.globalDeclarations.keySet()) {
	  ...
	  System.out.println("List of global declarations : "+this.globalDeclarations.get(declaration));
    }
  ...
  private Statement globaliseStatement(Statement s) throws Exception {
	  System.out.println("globaliseStatement");
  ...
  private AssignmentStatement globaliseAssignmentStatement(AssignmentStatement s) throws Exception {
	...
	System.out.println("oldDeclaration :"+oldDeclaration);
	System.out.println("newDeclaration :"+newDeclaration);
	
    Name newName = new Name(newDeclaration.vname);
    newName.setDeclaration(newDeclaration);
	newName.setType(newDeclaration.getType());
  ...
  private Expression globaliseExpression(Expression e) throws Exception {
	  System.out.println("Expression :"+e+" "+e.getClass());
  ...
  private Name globaliseName(Name e) throws Exception {
    ...
    Declaration globalisedDec = this.globalDeclarations.get(declaration);
	//EXPLANATION
	//WHEN A VARIABLE IS MEMBER OF STRUCT, GLOBAL_DECLARATION RETURNS STRUCT
	// THUS VARIABLE TYPE!=GLOBAL_DECLARATION TYPE
	// THEN WE FETCH THE MEMBERS OF STRUCT AND CHECK WITH VARIABLE NAME
	//IF IT MATCHES, WE ADD NEW DECLARATION FOR THE MEMEBER with appropriate type
	// but this could be a problem because this becomes a new variable instaed of a member of struct
	
	
	if(e.getType().getClass()!=globalisedDec.getType().getClass()){
		if(globalisedDec.getType().getClass()==(new Struct(null,null)).getClass())
		{
			for(Declaration d : ((Struct)globalisedDec.getType()).getDeclarationList()){
				
				if(d.vname.equals(e.getVarName()))
				{
					/*Declaration newdec = new Declaration(
					globalisedName.vname+"_"+d.vname,
					d.typeName,
					d.input);
					newdec.setType(d.getType().copy());
					System.out.println("New Declaration added: "+newdec.vname+" "+newdec.typeName);
					this.globalDeclarations.put(d,newdec);*/
					Name name=new Name(globalisedDec.vname+"_"+d.vname);
					name.setDeclaration(globalisedDec);
					System.out.println("d.type: "+d.getType());
					name.setType(d.getType());
					return name;
				}
				//	System.out.println("equal variables : "+e.getVarName() +" ---- "+d.vname
				//+ "-----" + globalisedName.vname+"_"+d.vname);
			}
		}
	}
	else
		System.out.println("equal type : "+globalisedDec.vname);
	//System.out.println("Comparing "+e+" and globalised fullVname "+fullVName+" globalised name "+globalisedName);
	//System.out.println("Type Comparing "+e.getType().getClass()+" and globalised type name "+globalisedName.getType().getClass());
	    Name name=new Name(globalisedDec.vname);
		name.setDeclaration(globalisedDec);
		name.setType(declaration.getType());
		return name;
