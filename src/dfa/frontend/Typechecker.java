  ...
  */
  ...
  private void typecheckName(Name name, Environment env) throws Exception {
    ...
    if(name.name.size() > 0) {
       ...
       */
	   System.out.println("inside if of env lookup typecheckName");
      ...
      Here,
        v:T belongs to env.
      */
        System.out.println("inside else of env lookup typecheckName :"+name.name.get(0)+"with declaration:"+name.getDeclaration() );
		System.out.println("type of name :"+name.getType().getClass());
		System.out.println("type of declaration :"+name.getDeclaration().getType().getClass());
		System.out.println("type of declaration full vname:"+name.getDeclaration().getFullVName('_'));
		
	if(name.getType().getClass()!=name.getDeclaration().getType().getClass()){
		if(name.getDeclaration().getType().getClass()==(new Struct(null,null)).getClass())
		{
			for(Declaration d : ((Struct)name.getDeclaration().getType()).getDeclarationList()){
				String str=name.getDeclaration().getFullVName('_')+"_"+d.vname;
				System.out.println("inside hello "+str+" "+name.name.get(0));
				
				if(str.equals(name.name.get(0)))
				{
	...
	else if(name.getType().getClass()==name.getDeclaration().getType().getClass()){
		dec=name.getDeclaration();
	}
	else	
        dec = env.lookup(name.name.get(0));
      }
	  System.out.println("declaration :"+dec);
      if(dec == null) {
